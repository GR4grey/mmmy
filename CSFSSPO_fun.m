function [b_tplus1_hat]=CSFSSPO_fun(data_close,data,tplus1, win_size,opts)
%{
...
x = -x_tplus1;
idx = find(x<=(min(x)+eplion));%最优资产编号所构成的集合

Imin_num = size(idx,2);%计算最优资产的个数
for i=1:Imin_num%给相应编号的资产向量赋值上1，表示等下将资金投资到这里
    I_vec(idx(i)) = 1;
end
b_tplus1_hat=I_vec./Imin_num;%将财富平均投资到最好资产上
